...
export class Exam {
  ...
  private findTeacherOverlap(teacher: Teacher): TORStatus {
    ...
    if (teacher.subjects.includes(this.subject)) {
      console.log(
        `Teacher ${teacher.name} teaches ${this.subject}, can't supervise this exam`,
      );
      return TORStatus.SubjectNotAllowed;
    }

    // Filter for the teacher's availability on the day of the exam
    const teacherAvail = teacher.availabilities.filter(
      (a) => a.dow === this.dow,
    );

    if (teacherAvail.length === 0) {
      console.log(
        `Teacher ${teacher.name} has no availability on day ${this.dow}`,
      );
    ...
    for (let i = 0; i < teacherAvail.length; i++) {
      ...
      console.log(
        `Exam time: ${this.start.toTimeString()} - ${this.end.toTimeString()}`,
      );
      console.log(
        `Overlap: ${latestStart.toTimeString()} - ${earliestEnd.toTimeString()}`,
      );
      ...
      if (latestStart < earliestEnd) {
        ...
        if (!this.overlapsWithExistingSegments(latestStart, earliestEnd)) {
          ...
          console.log(
            `Segment overlaps with existing booked segments, skipping`,
          );
        }
      } else {
    ...
    return foundValidSegment
      ? TORStatus.NoAvailability
      : TORStatus.NoAvailability;
  }
  ...
  private getLatestStartTime(avail: Availability): Date {
    ...
    availStartOnExamDay.setHours(
      avail.start.getHours(),
      avail.start.getMinutes(),
      0,
      0,
    );
    ...
    return new Date(
      Math.max(availStartOnExamDay.getTime(), this.start.getTime()),
    );
  }
  ...
  private getEarliestEndTime(avail: Availability): Date {
    ...
    availEndOnExamDay.setHours(
      avail.end.getHours(),
      avail.end.getMinutes(),
      0,
      0,
    );
  ...
  private isFullyCovered(): boolean {
    ...
    console.log(
      `Checking ${sortedSegments.length} segments for complete coverage`,
    );
    ...
    if (
      sortedSegments[sortedSegments.length - 1].end.getTime() <
      this.end.getTime()
    ) {
      console.log(
    ...
    for (let i = 1; i < sortedSegments.length; i++) {
      ...
      currentEnd = new Date(
        Math.max(currentEnd.getTime(), sortedSegments[i].end.getTime()),
      );
    }
